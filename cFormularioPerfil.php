...
# verificação de acesso da funcionalidade
...
if (isset($_GET["salvar"])) {
    ...
    } elseif (mb_strlen($nome) < 3 || mb_strlen($nome) > 30) {
        $mensagem = 'O nome informado é inválido. Deve conter entre 3 e 30 caracteres.';
    } elseif (mb_strlen($descricao) < 5 || mb_strlen($descricao) > 100) {
        $mensagem = "A descrição deve conter entre 5 e 100 caracteres.";
    } else {
        if(isset($_GET['codigo'])){
            // atualizar dados
            ...
            switch($resposta) {
                ...
                case 2:
                    ...
                    break;
                case 0:
                    $mensagem = "O nome do perfil é inválido. Deve conter no mínimo 3 caracteres.";
                    break;
                default:
                    $mensagem = "Ocorreu um erro ao atualizar o perfil. Verifique os dados e tente novamente.";
                    break;
            }
        } else {
            // cadastrar perfil
            ...
            switch($resposta) {
                ...
                case 0:
                    ...
                    break;
                default:
                    $mensagem = "Ocorreu um erro ao cadastrar o perfil. Verifique os dados e tente novamente.";
                    break;
            }
        }
    }
    ...
    if ($verificar == false && !isset($_GET['codigo'])) {
        header('Location: cMenu.php?msg=Acesso negado!');
        exit();
    }
}
...
if (is_array($funcionalidades)) {
    ...
    $aux .= "<input type='checkbox' name='funcionalidades'> Não há funcionalidades cadastradas no momento.<br>";
}
